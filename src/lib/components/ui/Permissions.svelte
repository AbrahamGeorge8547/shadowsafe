...
<div class="p-2 bg-[#3A4468]">
    ...
    <div class="mb-4 mt-4">
        <RadioGroup
            border="border border-[#4C598B4D]"
            rounded="rounded-container-token"
            display="flex-col"
            background="bg-[#3A4468]"
            class="px-4"
        >
            <RadioItem
                bind:group={value}
                ...
                value="MEMBER"
                rounded="rounded-2xl"
                class="flex flex-start text-sm my-2 py-[6px]"
                >View Only</RadioItem
            >
            <RadioItem
                bind:group={value}
                ...
                value="MANAGER"
                rounded="rounded-2xl"
                class="flex flex-start text-sm my-2 py-[6px]">Manage</RadioItem
            >
            <RadioItem
                bind:group={value}
                name="justify"
    ...
    <div class="flex flex-col ml-3">
        <button
            class="btn-md bg-[#4E46DC] rounded-3xl w-[70%] mb-4 text-sm"
            on:click={confirmPermission}
        >
            Confirm
        </button>

        <button
            class="btn-md bg-[#828CAE] rounded-3xl w-[70%] mb-2 text-sm"
            on:click={cancelDrop}
        >
            Cancel
        </button>
